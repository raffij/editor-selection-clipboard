  ...
  _load () {
    ...
    instance.on('selection-change', function (range, prevRange) {
      if (prevRange && prevRange.length) {
        block.quill2.instance.formatText(prevRange.index, prevRange.length, 'selection', false)
      }
      if (range && range.length) {
        block.quill2.instance.formatText(range.index, range.length, 'selection', true)
    ...
    instance.root.addEventListener('selectstart', function () {
      ...
      function onmousemove () {
        if (prevSelection) block.quill2.instance.formatText(prevSelection.index, prevSelection.length, 'selection', false)
        selection = instance.getSelection()
        block.quill2.instance.formatText(selection.index, selection.length, 'selection', true)
      }
...
function paragraph (editor, block) {
  ...
  if (!(block.quill2 instanceof Quill)) block.quill2 = new Quill()
  return html`<div style="position: relative">
    ${block.quill.render({ editor, block })}
    ${block.quill2.render({ editor, block })}
  </div>`
}
